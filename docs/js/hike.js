...
function createTrailAPIInfoElements(parentDiv) {
    ...
    let APIText = document.createTextNode(
        "All trail photos and information from https://www.hikingproject.com"
    );
    hikingAPIInfo.appendChild(APIText);
    parentDiv.appendChild(hikingAPIInfo);
    hikingAPIInfo = document.createElement("p");
    APIText = document.createTextNode(
        "Click on a trail's image to go to the hikingproject page"
    );
    hikingAPIInfo.appendChild(APIText);
    parentDiv.appendChild(hikingAPIInfo);
}
...
function createTrailLatLonPin(trail, newTrailDiv) {
    ...
    let trailLatLonText = document.createTextNode(
        "Lat: " + trail.latitude + " Lon: " + trail.longitude
    );
...
function createTrailDifficultyElements(trail, newTrailDiv) {
    ...
    difficultyButton.setAttribute(
        "title",
        "Learn more about trail difficulties"
    );
...
function createDirectionsElement(trail, newTrailDiv) {
    ...
    directionsNodeClone.querySelector("input[name='daddr']").value = trail.name;
...
function navigateToTrail(thisTrail) {
    console.log("thisTrail", thisTrail);
    ...
    fromLocation = fromNode.value;
    toLocation = toNode.value;

    // input validation and form submit
    if (!fromLocation) {
        window.alert("Enter a starting location to get directions.");
    } else if (!toLocation) {
        window.alert("Enter a destination location to get directions.");
    } else {
        formNode.submit();
    }
}
...
let getNearbyTrails = (latitude, longitude) => {
    ...
    hikingAPIURL =
        PROXY_URL +
        "https://hikingproject.com/data/get-trails?lat=" +
        latString +
        "&lon=" +
        lonString +
        "&maxDistance=15&key=200211115-d5a310327da9942ee690145b2d1ca5fc";
    startLoading();
    fetch(hikingAPIURL)
...
function filterTrails() {
    const bestChoiceIsEnabled = JSON.parse(
        sessionStorage.getItem("bestChoiceIsEnabled")
    );
    ...
    trailArray.forEach((trail, index) => {
        ...
        const canDisplay =
            !bestChoiceIsEnabled || trail.difficulty === bestChoice;
        if (canDisplay) {
